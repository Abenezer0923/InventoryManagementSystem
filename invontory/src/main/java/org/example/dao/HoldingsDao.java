...
public class HoldingsDao {
	...
	public Holdings findById(Integer id) {
	...
	public List<Holdings> listAll(Integer startPosition, Integer maxResult) {
		TypedQuery<Holdings> findAllQuery = em.createQuery(
				"SELECT DISTINCT h FROM Holdings h LEFT JOIN FETCH h.employee LEFT JOIN FETCH h.resources ORDER BY h.idholday",
				Holdings.class);
